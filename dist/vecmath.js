...
export function intersect2d(p1s, p1e, p2s, p2e) {
    const t = intersect2dT(p1s, p1e, p2s, p2e);
    if (t == null)
        return null;
    return vec2.lerp(vec2.create(), p1s, p1e, t);
}
export function intersect2dT(p1s, p1e, p2s, p2e) {
...
export function direction3d(ps, pe) {
    const dir = vec3.sub(vec3.create(), pe, ps);
    return vec3.normalize(dir, dir);
}
export function direction2d(ps, pe) {
    const dir = vec2.sub(vec2.create(), pe, ps);
    return vec2.normalize(dir, dir);
}
export function projectXY(p) { return vec2.fromValues(p[0], p[1]); }
export function projectXZ(p) { return vec2.fromValues(p[0], p[2]); }
export function projectYZ(p) { return vec2.fromValues(p[1], p[2]); }
export function intersect3d(p1s, p1e, p2s, p2e) {
    const dir1 = direction3d(p1s, p1e);
    const dir2 = direction3d(p2s, p2e);
    const p = (dir1[1] * dir2[0] - dir2[1] * dir1[0]) !== 0 ? projectXY :
...
export function reflectVec3d(out, id, n) {
...
export function mirrorBasis(out, mat, point, mirrorNormal, mirrorD) {
...
// }
export function projectionSpace(vtxs, n) {
    const a = vec3.create();
...
export function project3d(vtxs, normal) {
...
export function polygonNormal(vtxs) {
...
export function optimizeRotation(verts) {
}
