...
export class Range<T> {
  ...
  constructor(start: T, end: T, interpolator: Interpolator<T>) {
    ...
    this.points.push(new Point<T>(end, 1, left()));
  }

  public insert(val: T, t: number, interpolator: Interpolator<T>): void {
    const idx = binaryIndexOf1(this.points, new Point<T>(null, t, null), PointComparator);
...
export function binaryIndexOf1<T>(list: FastList<T>, searchElement: T, cmp: Comparator<T>): number {
