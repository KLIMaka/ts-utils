...
export class Rasterizer {
    ...
    bindAttribute(id, buf, offset, stride) {
        this.attrs[id] = buf;
        this.attrparams[id] = new BufferParams(offset, stride);
    }
    bindAttributes(startid, buf, numattrs) {
        for (let i = 0; i < numattrs; i++)
            this.bindAttribute(startid + i, buf, i, numattrs);
    }
    getIntersectionsTri(y, inter) {
        const reg = this.reg;
        for (var i = 0; i < 3; i++) {
            const v1 = reg[i];
    ...
    allocateRegisters(numverts) {
        const reg = new Array(numverts);
    ...
    *drawTriangles(indices, start = 0, length = indices.length) {
        const dx = this.dx;
        const dy = this.dy;
        const sx = this.sx;
        const sy = this.sy;
        ...
        const reg = this.reg;
        const ratrs = new Array(numattrs);
        const latrs = new Array(numattrs);
        const atrs = new Array(numattrs);
